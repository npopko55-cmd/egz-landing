...
Четыре последовательные фазы с жёсткой цепочкой зависимостей: сначала Google Таблица (нет зависимостей, разблокирует всё), затем Telegram-бот (нужны credentials от Таблицы и username для лендинга), затем лендинг (нужен username бота для deep link), затем рассылки (нужен рабочий end-to-end флоу). Продукт готов к промоушену после Phase 3; Phase 4 завершает жизненный цикл эфира.
...
- [ ] **Phase 1: Google Sheets** - Создать таблицу-регистратор и подключить интеграцию с SaleBot
- [ ] **Phase 2: Telegram Bot** - Настроить воронку SaleBot: welcome, регистрация, подтверждение, запись в Таблицу
- [ ] **Phase 3: Landing Page** - Разработать и задеплоить лендинг: дизайн через frontend-design, 4 блока, countdown, CTA → бот
...
**Goal**: Рабочая воронка SaleBot: пользователь по deep link запускает бота, нажимает «Зарегистрироваться», получает подтверждение, строка появляется в Таблице
**Depends on**: Phase 1
...
**Goal**: Лендинг задеплоен на GitHub Pages, соответствует дизайн-референсам, все CTA ведут в бот, countdown работает по МСК
**Depends on**: Phase 2
...
**Success Criteria** (what must be TRUE):
  ...
  3. Нажатие любой CTA-кнопки (hero, mid, final) открывает Telegram-бот по deep link `?start=landing`
...
|-------|----------------|--------|-----------|
| 1. Google Sheets | 0/TBD | Not started | - |
| 2. Telegram Bot | 0/TBD | Not started | - |
